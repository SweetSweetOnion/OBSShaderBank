...
uniform float inDuration = 2;
uniform float duration = 5;
uniform float outDuration = 1;
uniform float speed = 10;
uniform float noiseA = 0.1;
uniform float noiseB = 0.04;
uniform float noiseC = 0.2;
uniform float noiseD = 0.1;
uniform float noiseE = 0.2;
...
float4 render(float2 uv) {
	...
	float3 lightPos = float3(0,0.5,2+ builtin_elapsed_time_since_enabled*speed);
	...
	//d /= 10;
	float a = clamp(dif+0.3,0,1);


	float r = (sin(d*1.2+builtin_elapsed_time * 0.09)+1)*0.4;
	float g = (sin(d*1.2+builtin_elapsed_time * 0.12)+1)*0.3;
	float b = (sin(d*1.2+builtin_elapsed_time * 0.58)+1)*0.3;


	float4 col = float4(smoothstep(r,r+0.02,a),smoothstep(g,g+0.02,a),smoothstep(b,b+0.02,a),1);

	float f = (0.5+fade)*0.75;
	output = lerp(img,col,step(dif+0.2,0.5*f*f*f*f*f));
	//output.rgb = col;
	//output = img;
	//output = dif;
