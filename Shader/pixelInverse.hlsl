...
float4 render(float2 uv) {
	...
	int screenCordX = int(uv.x * float(builtin_uv_size.x));
	int screenCordY = int(uv.y * float(builtin_uv_size.y));
	float4 output = 0;
	float2 customUV = uv;
	if(screenCordX % 2 == 0){
		customUV.x = 1-customUV.x;
	}
	if(screenCordY % 2 == 0){
		customUV.y = 1-customUV.y;
	}
		customUV+= float2(random(builtin_elapsed_time)*0.5-0.5,random(builtin_elapsed_time	+100)*0.5-0.5) *0.03;
		output = image.Sample(builtin_texture_sampler, lerp(uv,customUV,fade)%1 );
