...
float4 render(float2 uv) {
	...
	float n2 = clamp(noise(uv*50 + builtin_elapsed_time)-0.2,0,1);
   ...
   blur /= iteration;
   diff /= iteration;
   diff = smoothstep(0.1,1,diff*5);


   //return diff;
	float2 off = float2(random(uv),random(uv+5))-0.5;

	float2 imgUV = uv + off*n*n*(1-diff)*0.03;

	float4 output = image.Sample(builtin_texture_sampler,uv);
	//output = blur*diff;

	output.rgb *= blur*diff;
	output.rgb *= 5;
