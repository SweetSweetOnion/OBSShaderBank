...
uniform float BPMx = 100;
uniform float BPMy = 200;
uniform float intensityX = 0.3;
uniform float intensityY = 0.5;
...
float4 render(float2 uv) {
	...
	imgUV.x += cos(builtin_elapsed_time_since_enabled * PI * (BPMx/60)) * y * intensityX;
	imgUV.y += cos(builtin_elapsed_time_since_enabled * PI * (BPMy/60)) *  intensityY;



	float4 output = image.Sample(builtin_texture_sampler,imgUV);
